"""Child-process execution shared by infrastructure controllers."""
...
RELEASE_NAME = "infrastructure-operations"
...
def job_argv(job: str, args: list[str]) -> list[str]:
    """Build exact argv for one private infrastructure job."""
    return [
        atlas_executable(),
        "job",
        "run",
...
def run_job(job: str, args: list[str]) -> int:
    """Run one job from the infrastructure release."""
