"""Active release and artifact lookup."""
...
from .manifests import ExecutableArtifact, ReleaseManifest, ServiceArtifact, load_manifest, validate_name
from .releases import read_version


@dataclass(frozen=True)
class ActiveRelease:
    """One release activated under the global current directory."""
...
def command_index(current_root: Path) -> dict[str, ExecutableRef]:
    ...
    for release in active_releases(current_root):
        for name, artifact in release.manifest.commands.items():
            ...
            index[name] = ExecutableRef(release=release, artifact_type="command", artifact=artifact)
    return index
...
def resolve_service(current_root: Path, release_name: str, service_name: str) -> ServiceRef:
