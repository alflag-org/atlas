...
def load_job_instance(jobs_dir: Path, name: str) -> JobInstance:
    ...
    release = validate_name(_string(raw, "release", f"job instance {name}"), kind="release")
    ...
    if not isinstance(environment_files, list) or not all(isinstance(item, str) for item in environment_files):
    ...
    if not isinstance(lock_value, str):
        raise ValueError(f"job instance {name}.lock must be a string")
