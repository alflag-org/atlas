"""Strict ``release.yml`` parsing and artifact models."""
...
class ReleaseManifest:
    """Validated contents of one release manifest."""
...
def _entrypoint(release_root: Path, value: str, label: str, suffix: str | None = None) -> Path:
    relative = Path(value)
...
def _parse_services(
    ...
    for raw_name, raw_entry in entries.items():
        ...
        if command is not None and (not isinstance(command, str) or command not in commands):
        ...
        if timer_value is not None and (not isinstance(timer_value, str) or not timer_value.strip()):
...
def load_manifest(release_root: Path) -> ReleaseManifest:
    ...
    _reject_unknown(raw, {"schema", "name", "commands", "jobs", "services"}, "release.yml")
    ...
    services = _parse_services(release_root, raw.get("services", {}), commands, jobs)
    return ReleaseManifest(name=name, commands=commands, jobs=jobs, services=services)
