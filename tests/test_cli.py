...
def make_bundle(tmp_path: Path, include_index: bool = True):
    ...
    if include_index:
        (payload_dir / "command-index.yml").write_text(
...
def _mock_signature_verification(monkeypatch):
    monkeypatch.setattr(
        release_mod, "verify_manifest_signature", lambda staged_dir: None
    )
...
def test_rollback(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "pull", str(b1), "--version", "v1"])
    assert main() == 0
    monkeypatch.setattr("sys.argv", ["atlas", "apply", "--version", "v1"])
    assert main() == 0
    monkeypatch.setattr("sys.argv", ["atlas", "pull", str(b2), "--version", "v2"])
    assert main() == 0
    monkeypatch.setattr("sys.argv", ["atlas", "apply", "--version", "v2"])
    assert main() == 0
    monkeypatch.setattr("sys.argv", ["atlas", "rollback"])
    assert main() == 0
...
def test_apply_failure_rolls_back_active_state_and_shims(monkeypatch, tmp_path):
    ...
    before_state = yaml.safe_load((var / "state.yml").read_text())
    ...
    monkeypatch.setattr(
        runtime_mod,
        "generate_shims",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("shim failure")),
    )
    ...
    after_state = yaml.safe_load((var / "state.yml").read_text())
...
def test_node_schema_missing_required_type_and_unknown_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "run", "hello"])
    with pytest.raises(
        ValueError, match=r"node\.yml: invalid key 'packs': expected type list"
    ):
        main()
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "run", "hello"])
    with pytest.raises(
        ValueError, match=r"node\.yml: invalid key 'extra': unknown key"
    ):
        main()
...
def test_secrets_schema_missing_type_and_unknown_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "run", "--materialize-secrets", "hello"])
    with pytest.raises(
        ValueError, match=r"secrets\.yml: invalid key 'oops': unknown key"
    ):
        main()
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "run", "--materialize-secrets", "hello"])
    with pytest.raises(
        ValueError, match=r"secrets\.yml: invalid key 'secrets': expected type list"
    ):
        main()
...
def test_invalid_yaml_has_file_and_location(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "run", "hello"])
    with pytest.raises(
        ValueError, match=r"invalid YAML in .*node\.yml line \d+, column \d+"
    ):
        main()
...
def test_apply_copies_only_active_pack_files(monkeypatch, tmp_path):
    ...
    hello.chmod(0o755)
    (
        payload_dir / "packs/base/files" / tmp_path.relative_to("/") / "managed.txt"
    ).parent.mkdir(parents=True)
    (
        payload_dir / "packs/base/files" / tmp_path.relative_to("/") / "managed.txt"
    ).write_text("ok")
    (
        payload_dir / "packs/other/files" / tmp_path.relative_to("/") / "other.txt"
    ).parent.mkdir(parents=True)
    (
        payload_dir / "packs/other/files" / tmp_path.relative_to("/") / "other.txt"
    ).write_text("ng")
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "pull", str(bundle2), "--version", "v2"])
    assert main() == 0
    monkeypatch.setattr("sys.argv", ["atlas", "apply", "--version", "v2"])
    assert main() == 0
...
def test_apply_rejects_files_symlink_traversal(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["atlas", "pull", str(bundle3), "--version", "v3"])
    assert main() == 0
...
def test_install_uninstall_systemd_dry_run(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["atlas", "install-systemd", "--dry-run", "--unit-dir", str(tmp_path)],
    )
    assert main() == 0
    monkeypatch.setattr(
        "sys.argv",
        ["atlas", "uninstall-systemd", "--dry-run", "--unit-dir", str(tmp_path)],
    )
    assert main() == 0
    out = capsys.readouterr().out
...
def test_update_reads_atlas_yml_and_no_apply(monkeypatch, tmp_path):
    _opt, var, etc = setup_node(monkeypatch, tmp_path)
    bundle = make_bundle(tmp_path)
    (etc / "atlas.yml").write_text(
        f"release:\n  source: file://{bundle}\n  version: v1\n  auto_apply: true\n"
    )
    monkeypatch.setattr("sys.argv", ["atlas", "update", "--no-apply"])
    assert main() == 0
    state_text = (var / "state.yml").read_text()
    assert "last_pull_at" in state_text
    assert "current_version" in state_text


def test_update_https_source_reports_not_supported(monkeypatch, tmp_path):
    _opt, _var, etc = setup_node(monkeypatch, tmp_path)
    (etc / "atlas.yml").write_text(
        "release:\n  source: https://example.invalid/release.tar\n  version: v1\n  auto_apply: true\n"
    )
    monkeypatch.setattr("sys.argv", ["atlas", "update"])
